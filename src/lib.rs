...
pub use bindings::{
    ...
    IRObjectType, IRRaytracingPipelineFlags, IRReflectionVersion, IRRootConstants,
    IRRootParameter1, IRRootParameter1__bindgen_ty_1 as IRRootParameter1_u, IRRootParameterType,
...
impl IRRootSignature {
    pub fn create_from_descriptor(
        ...
        unsafe {
            let mut error: *mut IRError = std::ptr::null_mut::<IRError>().add(0);
...
impl IRCompiler {
    pub fn new(path: impl AsRef<OsStr>) -> Result<IRCompiler, libloading::Error> {
        let compiler = unsafe { bindings::metal_irconverter::new(path) }?;
        let me = unsafe { compiler.IRCompilerCreate() };
        Ok(Self {
            funcs: Arc::new(compiler),
            me,
        })
    }
    ...
    pub fn alloc_compile_and_link(
        ...
        let mut error: *mut IRError = unsafe { std::ptr::null_mut::<IRError>().add(0) };
